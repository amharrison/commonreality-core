...
public class MockInterface {
	...
	public void keyPress(int keyCode) {
		...
		KeyEvent keyEvent = new KeyEvent(c, KeyEvent.KEY_PRESSED, System.currentTimeMillis(), modifiers, keyCode, keyChar );
	...
	public void keyRelease(int keyCode) {
		...
		int modifiers = 0;
		char keyChar = getKeyChar(keyCode);
		KeyEvent keyEvent = new KeyEvent(c, KeyEvent.KEY_RELEASED, System.currentTimeMillis(), modifiers, keyCode, keyChar );
...
//		System.err.println("move to " + _screenLocation);
		...
		_componentLocation = SwingUtilities.convertPoint(window, _componentLocation, component);
...
//		System.err.println("Currently over " + component);
		...
		if (_currentComponent != component) {
			...
			if (_currentComponent != null) {
				eventQueue.postEvent(new MouseEvent(_currentComponent, MouseEvent.MOUSE_EXITED,
						System.currentTimeMillis(), _currentMask, _componentLocation.x, _componentLocation.y,
						_screenLocation.x, _screenLocation.y, 0, false, MouseEvent.NOBUTTON));
				waitForIdle();
			}
			// enter
			if (component != null) {
				eventQueue.postEvent(new MouseEvent(component, MouseEvent.MOUSE_ENTERED, System.currentTimeMillis(),
						_currentMask, _componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y,
						0, false, _currentButtons));
				waitForIdle();
			}
		}
		...
		if (component != null) {
			if (_currentButtons == 0)
				me = new MouseEvent(component, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), _currentMask,
						_componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y, 0, false,
						MouseEvent.NOBUTTON);
			else {
				me = new MouseEvent(component, MouseEvent.MOUSE_DRAGGED, System.currentTimeMillis(), _currentMask,
						_componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y, 0, false,
						_currentButtons);
			}
	...
	public void mousePress(int modifiers, int button) {
		...
		MouseEvent me = new MouseEvent(_currentComponent, MouseEvent.MOUSE_PRESSED, System.currentTimeMillis(),
				modifiers, _componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y, 1, true,
				button);
...
//		System.err.println("press at " + _screenLocation + " over " + _currentComponent);
	...
	public void mouseRelease(int modifiers, int button) {
		...
		MouseEvent me = new MouseEvent(_currentComponent, MouseEvent.MOUSE_RELEASED, System.currentTimeMillis(),
				modifiers, _componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y, 1, false,
				button);

		MouseEvent clicked = new MouseEvent(_currentComponent, MouseEvent.MOUSE_CLICKED, System.currentTimeMillis(),
				modifiers, _componentLocation.x, _componentLocation.y, _screenLocation.x, _screenLocation.y, 1, false,
				button);
...
//			System.err.println("rootPane");
		...
		Component component = SwingUtilities.getDeepestComponentAt(root, location.x - rootWindow.getX(),
				location.y - rootWindow.getY());
		if (component == null) {
...
//				System.err.println("layeredPane");
				...
				component = SwingUtilities.getDeepestComponentAt(root, offset.x, offset.y);
//				/*
...
//				 */
				...
				if (component.getClass().getName().equals("reschu.game.view.PanelMsgBoard")) {
					System.err.println("Oops");
				}
	...
	protected Window findSmallestWindowAt(int x, int y) {
		...
		for (Window w : Window.getWindows()) {
			...
			if (!dim.contains(x, y) || !w.isVisible())
				continue;
